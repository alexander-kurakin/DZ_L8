...
{
    public class TurretPathTargetSelector : ITargetSelector
    {
        ...
        public TurretPathTargetSelector(
            Entity turretEntity,
        ...
        public Entity SelectTargetFrom(IEnumerable<Entity> targets)
        {
            ...
            _sectorEnemyQueryService.CollectEnemiesOnPlantablePath(sectorId.Index, _candidates);

            Entity closestDragon = null;
            float closestDragonDistance = float.MaxValue;
            Entity closestOther = null;
            ...
            for (int index = 0; index < _candidates.Count; index++)
            {
                ...
                if (TurretTargetPriority.IsDragon(candidate))
                {
                    if (distance < closestDragonDistance)
                    {
                        closestDragonDistance = distance;
                        closestDragon = candidate;
                    }
