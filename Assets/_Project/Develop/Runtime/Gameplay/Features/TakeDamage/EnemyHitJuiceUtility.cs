...
namespace Assets._Project.Develop.Runtime.Gameplay.Features.TakeDamage
{
    public static class EnemyHitJuiceUtility
    {
        ...
        public static void PlayOnTransform(Transform root, Vector3 spawnOrigin, float damage, TakeDamageVisualKind visualKind)
        {
            if (root == null)
                return;

            if (visualKind == TakeDamageVisualKind.Toxic)
                return;
